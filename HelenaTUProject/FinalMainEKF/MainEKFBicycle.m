...
for k = 2:N
    ...
    Q_bias = compute_Q_bias(x_ekf_all(:,k-1), alpha_v, alpha_h, Rpi2, acc_noise, gyro_noise, dt, alpha_bias);
    [x_pred, P_pred, Q] = ekf_prediction_step(x_ekf_all(:,k-1), P_pred, u, Q_bias, dt, f_bias, computeF_bias);
    x_pred(3) = max(x_pred(3),0); % cap velocity
    ...
    % Gem tilstand og hvilken opdatering der blev lavet
    x_ekf_all(:,k) = x_pred;
...
%         k = idx(j) + 1;
%         if ~shown(i+1)
%             h = plot(imuTable.Timestamp([k-1 k]), x_ekf_all(3, [k-1 k]), '-', ...
%                 'Color', colors{i+1}, 'LineWidth', 2, ...
%                 'DisplayName', labels{i+1});
%             shown(i+1) = true;
%         else
%             plot(imuTable.Timestamp([k-1 k]), x_ekf_all(3, [k-1 k]), '-', ...
%                 'Color', colors{i+1}, 'LineWidth', 2, ...
...
% 
% xlabel('Time'); ylabel('Velocity [m/s]');
% title('Velocity over time with correction types');
% legend('Location', 'best');
% set(gca, 'FontSize', 18); grid on;
% 
% 
...
%     x_ekf_all(1:2,:), imuTable.Timestamp, xGNSSRef, yGNSSRef, RuteRef.TimeUTC, 'EKF');
% %%
% colors = {[0.7 0.7 0.7], [0 0.6 0], [1 0 0], [0.5 0 0.5]};
% labels = {'Prediction only', 'BLE correction', 'GNSS correction', 'Both corrections'};
% 
% x_pos = x_ekf_all(1,:);
% y_pos = x_ekf_all(2,:);
...
% xlabel('East [m]');
% ylabel('North [m]');
% title('Estimated position with correction types');
% legend('Location', 'best');
% axis equal;
% set(gca, 'FontSize', 18); grid on;
